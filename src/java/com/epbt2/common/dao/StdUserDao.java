...
/**
 * StdUser Data Access Object (DAO). This class contains all database handling
 * that is needed to permanently store and retrieve StdUser object instances.
 */
/**
...
public class StdUserDao {
     ...
     * method in it!
     */
    public StdUser createValueObject() {
        return new StdUser();
    }
     ...
     */
    public StdUser getObject(Connection conn, String userId, String userPassword) throws NotFoundException, SQLException {

        StdUser valueObject = createValueObject();
        valueObject.setUserId(userId);
        valueObject.setUserPassword(userPassword);
     ...
     */
    public void load(Connection conn, StdUser valueObject) throws NotFoundException, SQLException {
        ...
        String sql = "SELECT * FROM SPBT.STD_USER WHERE (USER_ID = ? AND spbt.epbt.retr_password(USER_PASSWORD) = ? ) "
                + "UNION ALL "
                + "SELECT * FROM SPBT.STD_ADMIN WHERE (USER_ID = ? AND spbt.epbt.RETR_PASSWORD(USER_PASSWORD) = ? ) ";
        System.out.println(sql+":"+valueObject.getUserId()+":"+valueObject.getUserPassword());
        ...
        try {
            ...
            stmt.setString(4, valueObject.getUserPassword());
    ...
    public List loadAll(Connection conn) throws SQLException {
        ...
        String sql = "SELECT * FROM STD_USER ORDER BY USER_ID ASC ";
     ...
     */
    public synchronized void create(Connection conn, StdUser valueObject) throws SQLException {
        ...
        try {
            sql = "INSERT INTO STD_USER ( USER_ID, USER_PASSWORD, USER_GROUP_ID, "
                    + "USER_NAME, USER_CREATED, USER_STATUS) VALUES (?, ?, ?, ?, ?, ?) ";
            stmt = conn.prepareStatement(sql);

            stmt.setString(1, valueObject.getUserId());
            stmt.setString(2, valueObject.getUserPassword());
            stmt.setString(3, valueObject.getUserGroupId());
            stmt.setString(4, valueObject.getUserName());
            stmt.setString(5, valueObject.getUserCreated());
            stmt.setString(6, valueObject.getUserStatus());
     ...
     */
    public void save(Connection conn, StdUser valueObject)
            throws NotFoundException, SQLException {

        /*String sql = "UPDATE STD_USER SET USER_GROUP_ID = ?, USER_NAME = ?, USER_CREATED = ?, "
                + "USER_STATUS = ? WHERE (USER_ID = ? AND spbt.epbt.retr_password(USER_PASSWORD) = ? ) ";*/
        String sql = "UPDATE STD_USER SET "
                + "USER_PASSWORD = spbt.epbt.conv_password(?)  "
                + "WHERE (USER_ID = ? ) ";
        PreparedStatement stmt = null;

        try {
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, valueObject.getUserPassword());
     ...
     */
    public void delete(Connection conn, StdUser valueObject)
            throws NotFoundException, SQLException {
        ...
        String sql = "DELETE FROM STD_USER WHERE (USER_ID = ? AND spbt.epbt.retr_password(USER_PASSWORD) = ? ) ";
        PreparedStatement stmt = null;
    ...
    public void deleteAll(Connection conn) throws SQLException {
        ...
        String sql = "DELETE FROM STD_USER";
        PreparedStatement stmt = null;
     ...
     */
    public List searchMatching(Connection conn, StdUser valueObject) throws SQLException {

        List searchResults;

        boolean first = true;
        StringBuffer sql = new StringBuffer("SELECT * FROM STD_USER WHERE 1=1 ");
        ...
        if (valueObject.getUserPassword() != null) {
            ...
            sql.append("AND USER_PASSWORD LIKE '").append(valueObject.getUserPassword()).append("%' ");
        }
        ...
        if (valueObject.getUserCreated() != null) {
            if (first) {
                first = false;
            }
            sql.append("AND USER_CREATED LIKE '").append(valueObject.getUserCreated()).append("%' ");
        }

        if (valueObject.getUserStatus() != null) {
            if (first) {
                first = false;
            }
            sql.append("AND USER_STATUS LIKE '").append(valueObject.getUserStatus()).append("%' ");
        }
     ...
     */
    protected void singleQuery(Connection conn, PreparedStatement stmt, StdUser valueObject)
            throws NotFoundException, SQLException {
        ...
        try {
            ...
            if (result.next()) {
                ...
                valueObject.setUserCreated(result.getString("USER_CREATED"));
                valueObject.setUserStatus(result.getString("USER_STATUS"));
    ...
    protected List listQuery(Connection conn, PreparedStatement stmt) throws SQLException {
        ...
        try {
            ...
            while (result.next()) {
                StdUser temp = createValueObject();

                temp.setUserId(result.getString("USER_ID"));
                temp.setUserPassword(result.getString("USER_PASSWORD"));
                temp.setUserGroupId(result.getString("USER_GROUP_ID"));
                temp.setUserName(result.getString("USER_NAME"));
                temp.setUserCreated(result.getString("USER_CREATED"));
                temp.setUserStatus(result.getString("USER_STATUS"));
